...
// Typen um Callbacks als Lambda-Ausdruck (oder Funktionsobjekt) zu
// registrieren, wenn jemand die Maus im Fenster bewegt, bzw. klickt.
using mouse_moved_t = std::function<void(int x, int y)>;
using mouse_clicked_t =
    std::function<void(sf::Mouse::Button btn, int x, int y)>;
...
public:
     ...
     */
    void quit(int ms = 0);
    /** registriert ein Maus-Ereignis das feuert, falls die Maus
     * im Fenster bewegt wird.
     * Wird mit den Koordinaten x und y der Maus im Fenster aufgerufen.
     * Kann nicht deregistriert werden.
     * @param handler vom Typ mouse_moved_t
     */
    void register_mouse_event(mouse_moved_t handler) {
        mouse_events_moved.push_back(handler);
    }
    /** registriert ein Maus-Ereignis das feuert, falls die Maus
     * im Fenster geklickt wird.
     * Wird mit dem geklickten Button sowie den Koordinaten x und y
     * der Maus im Fenster aufgerufen.
     * Kann nicht deregistriert werden.
     * @param handler vom Typ mouse_clicked_t
     */
    void register_mouse_event(mouse_clicked_t handler) {
        mouse_events_clicked.push_back(handler);
    }
...
private:
    ...
    std::vector<mouse_moved_t> mouse_events_moved;
    std::vector<mouse_clicked_t> mouse_events_clicked;
};
