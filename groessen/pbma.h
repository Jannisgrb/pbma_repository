...
struct pbma_exception : public std::exception {
    std::string cause;
    explicit pbma_exception(const std::string& _cause);
    pbma_exception(const std::string& _cause, int val);
    pbma_exception(const std::string& _cause, long val);
    pbma_exception(const std::string& _cause, const std::string& val);
 ...
 * @param buffer binärer Inhalt der Datei
 */
void save_bytes(const std::string& filename, const std::vector<char>& buffer);

/** Vollständiges Schreiben einer Datei aus vector<string>
 * @param filename Dateiname der zu schreibenden Datei
 * @param lines Zeilen als Inhalt der Datei
 */
void save_lines(const std::string& filename,
                const std::vector<std::string>& lines);

/** Vollständiges Schreiben einer Datei aus vector<int>
 * @param filename Dateiname der zu schreibenden Datei
 * @param vals ints aus Ziffern als Inhalt der Datei
 */
void save_ints(const std::string& filename, const std::vector<int>& vals);

/** Vollständiges Schreiben einer Datei aus vector<long>
 * @param filename Dateiname der zu schreibenden Datei
 * @param vals longs aus Ziffern als Inhalt der Datei
 */
void save_longs(const std::string& filename, const std::vector<long>& vals);
 ...
 */
bool is_sorted(const std::vector<int>& a, int& first_error) noexcept;
...
public:
     ...
     * @param sdigits string-Wert, darf nur aus +/- am Anfang
                  und Dezimalziffern bestehen
     * @throws bei Eingabefehlern pbma_exception
     */
    explicit BigInt(const std::string& sdigits);
     ...
     * @param bi ganzzahliger Modulus
     * @return Geändertes ganzzahliges Objekt
     */
    BigInt& operator%=(const BigInt& other); // always positive, python %

    /** Vergleichsfunktion
     * @param bi zu vergleichende Ganzzahl
     ...
     * @return Zahl als long mit gleichem Wert
     */
    long to_long() const;

    /** Zurückkonvertierung zu einer long long-Zahl,
     * @throws pbma_exception, wenn Zahlbereich überschritten
     * @return Zahl als long long mit gleichem Wert
     */
    long long to_long_long() const;

    /** Zurückkonvertierung zu einer double Zahl mit Typkonvertierung
     * @throws pbma_exception, wenn Zahlbereich überschritten
     * @return Zahl als double mit fast gleichem Wert
     */
    double to_double() const {
        return static_cast<double>(to_long_long());
    }
 ...
 * @param bi die auszugebende Ganzzahl
