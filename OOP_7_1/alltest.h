...
// call that in tests to see whether there have been illegal writes
bool check_illegal_memory_write(void* p);

void check_memory(size_t scall, size_t acall);
inline void check_memory(int scall, int acall) {
    if (scall < 0 || acall < 0) {
        throw std::runtime_error("check_memory, cannot happen");
    }
    check_memory(static_cast<size_t>(scall), static_cast<size_t>(acall));
}
