...
// c++1[47] way, still ugly that the caller needs to use .c_str() on strings
...
bool file_exists(const std::string& filename) {
    const ifstream f(filename.c_str()); // automatically closed
    return f.good();
}
...
void save_lines(const std::string& filename, const vector<string>& lines) {
    ofstream os(filename);
    if (!os) {
        const char* s = filename.c_str();
        throw error(eformat("save_lines: filename=%s nicht schreibbar", s));
    }
    ...
    if (!os.good()) {
        const char* s = filename.c_str();
        throw error(eformat("save_lines: filename=%s Schreibfehler", s));
    }
    os.close();
}

void save_ints(const std::string& filename, const vector<int>& vals) {
    ofstream os(filename);
    if (!os) {
        const char* s = filename.c_str();
        throw error(eformat("save_ints: filename=%s nicht schreibbar", s));
    }
    ...
    if (!os.good()) {
        const char* s = filename.c_str();
        throw error(eformat("save_ints: filename=%s Schreibfehler", s));
    }
    os.close();
}

void save_longs(const std::string& filename, const vector<long>& vals) {
    ofstream os(filename);
    if (!os) {
        const char* s = filename.c_str();
        throw error(eformat("save_longs: filename=%s nicht schreibbar", s));
    }
    for (const auto& val : vals) {
        os << val << "\n";
    }
    if (!os.good()) {
        const char* s = filename.c_str();
        throw error(eformat("save_longs: filename=%s Schreibfehler", s));
    }
    os.close();
}
...
static void read_accept_str_vec(const string& line, const string& accept,
    ...
    while ((pos = line.find_first_of(chars_delim, prev)) != string::npos) {
        if (pos > prev) {
            ...
            if (!word.empty() && // nonempty, only of acceptable chars
    ...
    const string word = line.substr(prev);
...
vector<double> read_doubles(const std::string& filename) {
    ...
    for (const string& dword : read_accept_file(filename, CHARS_FLOAT_WORDS)) {
        try {
            ...
            const auto s = dword.c_str();
            const auto e = err.what();
...
static std::vector<int> read_pgm5(const string& filename) {
    const vector<char> raw = read_bytes(filename); // binary format
    if (raw[0] != 'P' || raw[1] != '5') {
    ...
    if (cur == -1) {
        const auto s = filename.c_str();
        const auto e = "keine Breite/Hoehe/Helligkeit";
    ...
    const size_t needed =
        static_cast<size_t>((twobyte ? 2 : 1) * width * height);
    if (len_bytes != needed) {
        ...
        const auto s = filename.c_str();
        const auto ef =
            eformat(fmt, s, width, height, maxbright, len_bytes, needed);
        throw error(ef);
    }
    ...
    for (int i = 1; i < argc; i += 1) {
        ...
        if (starts_with(arg, "-") and
            ...
            if (it != string::npos) { // with equals sign -> option
                const string key = arg.substr(0, it);
                const string val = arg.substr(it + 1);
                _options[key] = val;
            } else { // -> flag
...
int args_t::int_option(const std::string& key, int defval) const {
    ...
    try {
        ...
        const auto msg = "args::int_option(%s, %d): no int %s";
        const auto k = key.c_str();
        const auto s = it->second.c_str();
...
long args_t::long_option(const std::string& key) const {
    ...
    try {
        size_t idx;
        const long val = stol(it->second, &idx);
        if (it->second[idx] != 0) {
...
double args_t::double_option(const std::string& key) const {
    ...
    try {
        size_t idx;
        const double val = stod(it->second, &idx);
        if (it->second[idx] != 0) {
...
int args_t::int_pos(size_t idx) const {
    ...
    try {
        ...
        const auto s = _positionals[idx].c_str();
...
vector<int> create_randints(int how_many, int lower, int upper) {
    ...
    const auto seed = beginning.time_since_epoch().count() % 2097152;
...
bool is_sorted(const vector<int>& a, int& first_error) noexcept {
...
bool check_sort_one(sort_function sort, int size, bool timing, int verbose) {
    ...
    reset_swaps();
    sort(a);
    const long swaps = get_swaps();
    const string measure = timer.human_measure();
    if (!is_sorted(a, first_error)) {
...
bool check_sort(sort_function sort, bool timing, bool large, int verbose) {
    vector<int> sort_sizes = {10, 100, 1000, 10000, 20000,
    ...
    if (large) {
        vector<int> large_sort_sizes = {200000, 400000, 600000, 800000,
...
long _swaps_counter = 0; // not static, as inlined in header in swap

long get_swaps() noexcept {
    return _swaps_counter;
}

long reset_swaps() noexcept {
    const long ret = _swaps_counter;
    _swaps_counter = 0;
    return ret;
}
...
BigInt::BigInt(const std::string& sdigits) {
    sign = false; // only to please Eclipse, it is assigned in ss >> *this
    if (!is_decimalstring(sdigits)) {
        cerr << "BigInt(string): ungueltige Zahl, " << sdigits << endl;
        throw pbma_exception("BigInt(string): ungueltige Zahl, ", sdigits);
    }
    stringstream ss(sdigits);
    ss >> *this;
}
...
static inline string add(const string& d1, const string& d2) {
    vector<char> res;
    auto it1 = d1.crbegin();
    const auto it1e = d1.crend();
    auto it2 = d2.crbegin();
    const auto it2e = d2.crend();
    char carry = '0';
...
static inline pair<int, string> subtract(const string& d1, const string& d2) {
    const int cmp = compare_digitstrings(d1, d2);
    if (cmp < 0) {
...
BigInt& BigInt::operator+=(const BigInt& other) {
    if (sign == other.sign) {
        ...
        if (sign == -1) {
            const auto p = subtract(other.digits, digits);
            sign = p.first;
            digits = p.second;
        } else { // other.signs == -1
            const auto p = subtract(digits, other.digits);
            sign = p.first;
            digits = p.second;
        }
    }
...
static inline string mult(const string& ds, char digit, int shift) {
    vector<char> res;
    const int val = digit - '0';
    int carry = 0;
    for (auto it = ds.crbegin(); it != ds.crend(); ++it) {
        const int cur = *it - '0';
        const int mul = (cur * val) + carry;
        carry = mul / 10;
        res.push_back(static_cast<char>((mul % 10) + '0'));
    }
    if (carry > 0) {
        if (carry > 9) {
            const string msg = "BigInt: mult, carry too large???";
...
static inline string mult(const string& d1, const string& d2) {
    string res = "0";
    const int d2size = static_cast<int>(d2.size());
    for (int i = 0; i < d2size; ++i) {
        // start with last
        const char digit = d2[static_cast<size_t>(d2size - 1 - i)];
...
static inline bool does_fit(const string& d1, const string& d2) {
    ...
    if (d1.size() != d2.size()) {
        const string msg = "BigInt, does_fit: d1.size() != d2.size()";
...
static inline string divide(const string& d1, const string& d2, int sign) {
    ...
    string working = "0";
    vector<char> res;
    for (const char digit : d1) {
        if (working[0] == '0') { // no leading zeros but single 0
            working = digit;
        } else {
            working += digit;
        }
        const int fit = fit_it(multiples, working);
        if (fit == 0) {
            if (!res.empty()) {
        ...
        const auto p = subtract(working, multiples[static_cast<size_t>(fit)]);
        if (p.first != 1) { // cannot be negative
            const string msg = "BigInt: divide, subtract must not be negative";
            throw runtime_error(msg + " internal error");
        }
        working = p.second;
...
BigInt& BigInt::operator/=(const BigInt& other) {
    ...
    sign *= other.sign;
    const int res = compare_digitstrings(digits, other.digits);
    if (res < 0) {
        if (sign == -1) {
...
BigInt& BigInt::operator%=(const BigInt& other) {
    ...
    // res > 0
    const string sdiv = divide(digits, other.digits, 1);
    const string smul = mult(sdiv, other.digits);
    if (digits == smul) {
        sign = 1;
        digits = "0";
        return *this;
...
int BigInt::compare(const BigInt& other) const {
    ...
    // same sign
    const int cmp = compare_digitstrings(digits, other.digits);
    return cmp * sign;
}
