...
// Konstruktor
template <typename T>
Sequence<T>::Sequence(size_t cap) : _cap{cap}, _size{0} {
...
void Sequence<T>::reserve(size_t new_cap) {
    if (new_cap < _size) return; // Kann nicht kleiner als aktuelle Elementanzahl werden
...
const T& Sequence<T>::operator[](size_t pos) const {
    if (pos >= _size) {
        throw std::runtime_error("const Sequence::operator[]: pos out of bounds");
    }
    return _array[pos];
}
